...
int snd_subsystem_sp_poweron(void)
{
    ...
    if (is_voice_call_mode) {   // voice call mode
            snd_sybsystem_SP_cfgInp(SP_MONO_DIFFERENTIAL, /*5*/10);   // 20130416 hdj Speaker preAMP gain = 4.5dB -> 12dB
            snd_subsystem_setSPGain(/*2*/1);                          // 20130416 hdj 24dB-> 20dB
            snd_subsystem_setSPAtt(/*75*/51);                         // 20130416 for hardware tune Speaker volume = -15dB 
            snd_subsystem_enSPZC(SP_ZCS_ENABLE);
            snd_subsystem_enSPVRamp(SP_SV_ENABLE);
            snd_subsystem_cfgDRC(/*DRC_MODE_OFF*/DRC_MODE_2TO1, NCLIP_LIMIT_ON); //20130416 hdj DRC enable
    	    ...
    	    snd_sybsystem_SP_cfgInp(SP_MONO_DIFFERENTIAL, /*5*/7);	   // 20130416 hdj Speaker preAMP gain = 4.5dB -> 7.5dB
		snd_subsystem_setSPGain(1);	// Speaker gain = 20dB
    		snd_subsystem_setSPAtt(/*53*/51);	                        // 20130416 for hardware tune Speaker volume =-14dB ->  -15dB 
		snd_subsystem_enSPZC(SP_ZCS_ENABLE);
		snd_subsystem_enSPVRamp(SP_SV_ENABLE);
