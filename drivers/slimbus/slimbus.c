			...
			enum slim_ch_control chctrl, bool commit)
{
	...
	mutex_lock(&ctrl->m_ctrl);
	do {
#if 1 // 20130320 Jimmy, 174534 Avoid same channel definition, removal in same sequence 
		struct slim_pending_ch *pch;
...
#if 0 // 20130320 Jimmy, 174534 Avoid same channel definition, removal in same sequence 
			ret = add_pending_ch(&sb->mark_removal, chan);
			if (ret)
				break;
#else // here is patch block
